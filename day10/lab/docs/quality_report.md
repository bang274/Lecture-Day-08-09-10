...
| Expectation halt? | — | Không | Tất cả 9 expectations PASS trên clean run |
...
**Trước (inject-bad — stale 14 ngày):** expectation `refund_no_stale_14d_window` FAIL (1 violation) → chunk chứa "14 ngày làm việc" vẫn được embed. Eval trên inject-bad sẽ có `hits_forbidden=yes` vì top-k vẫn chứa chunk stale.

---

## 3. Freshness & monitor

**Kết quả:** `freshness_check=FAIL` (age=116.7h, SLA=24h)

**Giải thích:** CSV mẫu có `exported_at=2026-04-10T08:00:00` — cũ hơn 24h so với thời điểm run. Đây là **FAIL hợp lý** — đúng với kịch bản slide 3 Day 10: "data từ database công ty đột nhiên sai". Trong production, nhóm sẽ:
